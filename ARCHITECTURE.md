...
> System design and component breakdown for the Virtual Audio Cable application
...
|-----------|------|-------------|
| **App.tsx** | `src/App.tsx` | Main application with tab navigation |
| **RoutingMatrix** | `src/components/RoutingMatrix.tsx` | N×M routing grid with volume/mute controls |
| **VuMeter** | `src/components/VuMeter.tsx` | Real-time audio level visualization |
| **DeviceList** | `src/components/DeviceList.tsx` | Audio device selection interface |
| **FxChain** | `src/components/FxChain.tsx` | Per-channel DSP configuration |
| **Presets** | `src/components/Presets.tsx` | Save/load routing configurations |
...
|--------|------|-------------|
| **main.rs** | `src/main.rs` | Application entry point, Tauri setup |
| **audio.rs** | `src/audio.rs` | Audio capture engine using cpal |
| **commands.rs** | `src/commands.rs` | Tauri IPC command handlers |
| **config.rs** | `src/config.rs` | Application configuration |
| **devices.rs** | `src/devices.rs` | Virtual device management |
| **dsp.rs** | `src/dsp.rs` | Audio processing pipeline |
| **routing.rs** | `src/routing.rs` | Routing matrix logic |
...
| **Noise Gate** | Suppress low-level noise | Threshold, enabled/disabled |
| **5-Band EQ** | Shape frequency response | 60Hz, 250Hz, 1kHz, 4kHz, 16kHz |
| **Compressor** | Control dynamic range | Threshold, ratio, attack, release |

---

## 🖥️ Platform-Specific Audio Drivers

### macOS

- **Framework**: CoreAudio
- **Driver**: BlackHole virtual audio driver
- **Custom**: AudioServerPlugin for production builds

### Windows

- **API**: WASAPI (Windows Audio Session API)
- **Driver**: VB-Cable or custom WDM kernel driver
- **Loopback**: System audio capture support

### Linux

- **Modern**: PipeWire (userspace, no kernel driver required)
- **Legacy**: PulseAudio
...
For more details, see [DEVELOPMENT.md](DEVELOPMENT.md) or [INSTALL.md](INSTALL.md)
